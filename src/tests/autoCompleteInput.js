...
class AutocompleteExample extends Component {
    static renderFilm(film) {
        ...
        const roman = episode_id < ROMAN.length
            ? ROMAN[episode_id]
            : episode_id;
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                <Autocomplete autoCapitalize="none" autoCorrect={false} containerStyle={styles.autocompleteContainer} data={films.length === 1 && comp(query, films[0].title)
                    ? []
                    : films} defaultValue={query} onChangeText={text => this.setState({query: text})} placeholder="Enter Star Wars film title" renderItem={({title, release_date}) => (
                    <TouchableOpacity style={{
                        flexDirection: "row",
                        alignItems: 'center'
                    }} onPress={() => this.setState({query: title})}>
                        <Image source={{
                            uri: 'http://placehold.it/100x100'
                        }} style={{
                            height: 40,
                            width: 40,
                            borderRadius: 20
